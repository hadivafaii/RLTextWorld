  ...
  for nb_objects in 10 20 40 60; do
    ...
    for quest_length in $(seq 5 5 ${max_quest_length}); do
      for goal in 'brief' 'detailed'; do
        ./mk_train.sh $base_dir $goal $world_size $nb_objects $quest_length; sem --wait
        ./mk_valid.sh $base_dir $goal $world_size $nb_objects $quest_length; sem --wait
        ./mk_test.sh $base_dir $goal $world_size $nb_objects $quest_length; sem --wait
      done
    done
  done
done
