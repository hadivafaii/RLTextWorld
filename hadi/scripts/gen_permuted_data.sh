...
do
  ...
  if [ -z "$game_spec" ]; then # if game_spec is NULL
    screen -S "get_permuted_${ADDR[0]}_${ADDR[1]}_$pretrain_mode" -X stuff "python3 gen_pretrain_data.py $game_type $pretrain_mode --k 3 ^M"
  else
    screen -S "get_permuted_${ADDR[0]}_${ADDR[1]}_$pretrain_mode" -X stuff "python3 gen_pretrain_data.py $game_type $pretrain_mode --k 3 --game_spec $game_spec ^M"
  fi
done
