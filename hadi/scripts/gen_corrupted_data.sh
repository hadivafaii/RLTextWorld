...
do
  ...
  if [ -z "$game_spec" ]; then # if game_spec is NULL
    ...
    screen -S "get_corrupted_$pretrain_mode" -X stuff "python3 gen_pretrain_data.py $game_type $pretrain_mode --mask_prob 0.3 --seeds 110 121 332 443 554 665  --game_spec $game_spec ^M"
  fi
done
